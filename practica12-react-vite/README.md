...
- **`src/App.jsx`**: el componente principal — una función que devuelve JSX (HTML mezclado con JavaScript) describiendo lo que se ve en pantalla.
...
En este proyecto se personalizó `App.jsx` para mostrar el nombre del alumno y la materia, además del contador:
...
Cada clic en el botón llama `setCount((count) => count + 1)`, que le dice a React "toma el valor actual y súmale 1". React reprograma el render y el texto del botón se actualiza solo.
...
**Antes de hacer clic (`Contador: 0`):**

![Antes de hacer clic](./docs/01-antes-clic.png)

**Después de varios clics (`Contador: 37`):**
